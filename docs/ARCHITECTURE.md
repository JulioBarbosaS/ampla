...
- **Sandbox mode (`sandbox: "docker"`, opt-in via `ampla connect --sandbox`):** the daemon runs each `claude -p` in an **ephemeral container** (`bridge/sandbox/Dockerfile`) that bind-mounts **only the project dir** (ro, or rw with `allow_write`) plus the Claude credential. The rest of the host filesystem **does not exist** inside — kernel-enforced confinement, the real boundary the deny-rules can't give. Network stays up (claude needs the Anthropic API), but with no `Bash`/`WebFetch`/`WebSearch` the model has no way to reach anywhere else; an egress-allowlist proxy is a future hardening.
