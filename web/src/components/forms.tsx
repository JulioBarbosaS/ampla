...
export function Field({
  ...
  return (
    <label className="block">
      ...
      <input
        {...props}
        className="w-full rounded-md border border-zinc-700 bg-zinc-800 px-3 py-2 text-sm text-zinc-100 outline-none transition-colors placeholder:text-zinc-500 focus:border-amber-500"
      />
    </label>
  );
}
...
export function AuthCard({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className="flex min-h-screen items-center justify-center bg-zinc-950 px-4">
      <div className="w-full max-w-sm rounded-2xl border border-zinc-800 bg-zinc-900 p-7 shadow-xl shadow-black/40">
        <div className="flex flex-col items-center gap-2 text-center">
          <Logo variant="full" className="h-11" />
          <h1 className="text-sm font-medium text-zinc-400">{title}</h1>
        </div>
        <div className="mx-auto my-5 h-px w-2/3 bg-zinc-800" />
        {children}
      </div>
    </div>
  );
